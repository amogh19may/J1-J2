...
program calling_func
   ...
   interface nbr_vec
        function nbr_vec(n_dim)
            integer ::n_dim
            integer ::nbr_vec(8,n_dim**2)
        end function nbr_vec
   end interface

   interface sub_latt
    function sub_latt(n_dim)  #error rep:n_din->n_dim
        integer ::n_dim
	integer ::sub_latt(4,n_dim**2/4)
   ...
   integer,parameter::n_save=100
   integer,parameter::n_eqb=100
   ...
   real:: Q(n_dim**2),prop1(rel),prop2(rel),recv_buff(4*n_dim**2),send_buff(n_dim**2),prop_send(4),prop_recv(16)

   nbr_all=nbr_vec(n_dim)
   latt=sub_latt(n_dim)


   #removed unallocated array, replace by fixed size
   call MPI_Init(ierr)
   call MPI_Comm_rank(MPI_COMM_WORLD,ranks,ierr)
   call MPI_Comm_size(MPI_COMM_WORLD,n_size,ierr) #error rep:Comm->COMM
   n_proc=n_dim**2/(16)
		...
		recv_buff=0.0
                if(ranks==0) then
                    do ir=1,n_proc
                        ...
                        Q(latt(isbl,ir))=Q(latt(isbl,ir))+flip(latt(isbl,ir),Q,T_arr(k),J2,n_dim,nbr_all,pi)
                    end do
                    ...
                    do ir=n_proc*ranks+1,n_proc*(ranks+1)
                        ...
                        send_buff(latt(isbl,ir))=flip(latt(isbl,ir),Q,T_arr(k),J2,n_dim,nbr_all,pi)
...
function delE1(i,nbr,Qmat,J2,w,n_dim) #removed twist
    implicit none

    real,dimension(n_dim**2) :: Qmat
    integer ::i, n_dim
    real ::q, qr, ql, qd, qu, Ei, Ef, w, delE1, J2, qn, qs, qw, qe
...
function flip(ind,Q,T,J2,n_dim,all_nbr,pi)#removed twist
    implicit none

    integer:: n_dim,i,j,ind
    integer::all_nbr(8,n_dim**2)
    real :: T,pi,J2,dE,w,p,delE1,flip,Tin
    real,dimension(n_dim**2) :: Q
	    ...
	    Tin=1/T
            flip=0.0
            dE=delE1(ind,all_nbr,Q,J2,w,n_dim)
...
function loc_field(i,Qmat,J2,nbr,n_dim)
        ...
        q=Qmat(i) # changed order (not needed)
        qu=Qmat(nbr(4,i))
        qd=Qmat(nbr(8,i))
        ql=Qmat(nbr(2,i))
        qr=Qmat(nbr(6,i))
