...
program calling_func
   ...
   integer,parameter::n_save=500000
   integer,parameter::n_eqb=500000
   real(db),parameter::pi=4.0_db*atan(1.0_db)
   real(db),parameter::J2=0.7_db
   integer::i,j,k,ranks,n_size,n_proc,ierr,istat,isbl,ir,lovr,lmc,lm2
   real(db) :: T_arr(100)=[(i,i=1,100,1)]*0.0002_db+0.55_db
   real(db) :: avp1,avp2,avp3,avE,avV,M1x,M1y,M2x,M2y,M,chi,E,chiral,energy,flip,loc_field
   integer::nbr_all(8,n_dim**2),latt(4,n_dim**2/4)
   real(db):: Q(n_dim**2),prop1(rel),prop2(rel),send_buff(n_dim**2),stag(n_dim**2),prop_send(4),prop_recv(16)
   real(db),allocatable::recv_buff(:)
   ...
   do k=61,70
	...
	M=0.0_db
        ...
        call MPI_Bcast(Q,n_dim**2,MPI_DOUBLE_PRECISION,0,MPI_Comm_WORLD,ierr)
		...
		recv_buff=0.0_db
                if(ranks==0) then
                    do ir=1,n_proc
                        ...
                        Q(latt(isbl,ir))=Q(latt(isbl,ir))+flip(latt(isbl,ir),Q,T_arr(k),J2,n_dim,nbr_all,pi,0.0_db)
                    end do
                    ...
                    do ir=n_proc*ranks+1,n_proc*(ranks+1)
                        ...
                        send_buff(latt(isbl,ir))=flip(latt(isbl,ir),Q,T_arr(k),J2,n_dim,nbr_all,pi,0.0_db)

                    end do

                end if
call MPI_Gather(send_buff,n_dim**2,MPI_DOUBLE_PRECISION,recv_buff,n_dim**2,&
...
MPI_DOUBLE_PRECISION,0,MPI_Comm_WORLD,ierr)
        ...
        if(ranks==0 .or. ranks==1) then
            prop1(j)=1-n_save*avp3/(30.0_db*(avp2**2))
	    prop2(j)=(10.0_db*avp2/n_save-(10.0_db*avp1/n_save)**2)*n_dim**2/(T_arr(k))
	else if(ranks==2 .or. ranks==3) then
	    prop1(j)=avp3/avp2-10.0_db*avp1/n_save
            prop2(j)=10.0_db*avp2/n_save
        end if
    ...
    call MPI_Gather(prop_send,4,MPI_DOUBLE_PRECISION,prop_recv,4,&
    MPI_DOUBLE_PRECISION,0,MPI_Comm_WORLD,ierr)
    if(ranks==0) then
...
function delE1(i,nbr,Qmat,J2,w,n_dim,twist)
    implicit none
    integer,parameter::db=selected_real_kind(8)
    real(db),dimension(n_dim**2) :: Qmat
    integer ::i, n_dim
    real(db) ::q, qr, ql, qd, qu, Ei, Ef, w, delE1, J2, qn, qs, qw, qe,twist
...
function flip(ind,Q,T,J2,n_dim,all_nbr,pi,twist)
    ...
    real(db) :: T,pi,J2,twist,dE,w,p,delE1,flip,Tin
    real(db),dimension(n_dim**2) :: Q
...
function loc_field(i,Qmat,J2,nbr,n_dim)
        implicit none
        integer,parameter::db=selected_real_kind(8)
        real(db)::q,qr,ql,qu,qd,qs,qn,qw,qe,sx,sy,loc_field,J2
        real(db)::Qmat(n_dim**2)
...
function energy(Q,adj,n_dim,J2)
	implicit none
    integer,parameter::db=selected_real_kind(8)
	real(db)::Q(n_dim**2),J2,energy,qe,q1,q2,q3,q4
