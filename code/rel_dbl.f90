...
program hello
    ...
    real(db),parameter::pi=4.0*atan(1.0)
    real(db),parameter::J2=0.70
    integer::ranks,proc,ierr
    integer::i,j,k,l,k1,k2,k3,k4,a,b
    integer::nbr(8,n_dim**2),sb_lt(4,(n_dim**2)/4),nbr_full(8,2)
    real(db)::T,T_in,mc,ovr,chiral,chi4,chi2
    real(db)::Q(n_dim**2),U_all(rel)
    real(db),allocatable::U(:)
    call MPI_Init(ierr)
    ...
    do l=1,rel/proc
        ...
        do i=1,n_save/2
            ...
            if(modulo(i,5)==0) then
                chi4=chi4+chiral(Q,n_dim,nbr)**4
                chi2=chi2+chiral(Q,n_dim,nbr)**2
            end if
        end do
        U(l)=1-0.10_db*n_save*chi4/(3*chi2**2)
    end do
    call MPI_Gather(U,rel/proc,MPI_DOUBLE_PRECISION,U_all,rel/proc,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
    if(ranks==0) then
...
function mc(Q,ind,nbr,T_in,J2,n_dim,pi)
    ...
    real(db)::T_in,J2,pi,wf,wi,Ei,Ef,r,mc
    real(db)::Q(n_dim**2),p(8)
    wi=Q(ind)
    call random_number(wf)
    call random_number(r)
    wf=wf*2*pi-pi
    p=Q(nbr(1:8,ind))
    Ei=-1.0_db*(cos(wi-p(1))+cos(wi-p(3))+cos(wi-p(5))+cos(wi-p(7)))+J2*(cos(wi-p(2))+cos(wi-p(4))+cos(wi-p(6))+cos(wi-p(8)))
    Ef=-1.0_db*(cos(wf-p(1))+cos(wf-p(3))+cos(wf-p(5))+cos(wf-p(7)))+J2*(cos(wf-p(2))+cos(wf-p(4))+cos(wf-p(6))+cos(wf-p(8)))
...
function ovr(Q,ind,nbr,T_in,J2,n_dim,pi)
    ...
    integer::n_dim,ind
    integer::nbr(8,n_dim**2)
    real(db)::T_in,J2,pi,wi,ovr,hloc_x,hloc_y
    real(db)::Q(n_dim**2),p(8)
    wi=Q(ind)
    p=Q(nbr(1:8,ind))
    hloc_x=-cos(p(1))-cos(p(3))-cos(p(5))-cos(p(7))+J2*(cos(p(2))+cos(p(4))+cos(p(6))+cos(p(8)))
    hloc_y=-sin(p(1))-sin(p(3))-sin(p(5))-sin(p(7))+J2*(sin(p(2))+sin(p(4))+sin(p(6))+sin(p(8)))
...
function chiral(Q,n_dim,nbr)
    ...
    do i=1,n_dim**2
        ...
        qk=Q(nbr(8,i))
        ql=Q(nbr(1,i))
        chiral=chiral+0.25_db*(cos(qi-qj)-cos(qi-ql)-cos(qk-qj)+cos(qk-ql))
    end do
